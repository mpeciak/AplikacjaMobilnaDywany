...
{
    public abstract class AItemViewModel<T> : BaseViewModel
    {
        ...
        async void OnItemSelected(T item)
        {
            ...
            //await Shell.Current.GoToAsync($"{nameof(DywanDetailPage)}?{nameof(DywanDetailViewModel.ItemId)}={item.IdDywanu}");
        }
    }
}
